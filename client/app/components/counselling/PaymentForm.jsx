...
const PaymentForm = ({ id, isGroup, amount, setConfirmationMessage, setStep }) => {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      const uploadRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/upload-screenshot`, {
        method: "POST",
        body: formData,
      });
  ...
  useEffect(() => {
    const fetchQr = async () => {
      try {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/payment-qr/${amount}`);
        if (res.data.success) {
